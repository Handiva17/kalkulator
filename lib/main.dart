...
class Calculator extends StatefulWidget { //Class ini adalah widget stateful karena memiliki nilai yang selalu berubah ketika tombolnya ditekan
  @override
...
class _CalculatorState extends State<Calculator> {
      ...
      if (buttonText == "C") { // jika tombol c ditekan, output diubah menjadi 0 
        ...
        try {
          output = evaluateExpression(
            output.replaceAll('x', '*').replaceAll('÷', '/')); // evaluateExpression adalah fungsi yang digunakan utk menghitung hasil dari expression
        } catch (e) { // catch digunakan untuk menangani kesalahan yang terjadi ketika menghitung expression (contohnya jika user)
          output = "Error";
        }
      } else {
        if (output == "0") {
  ...
  Widget buildButton(String buttonText, Color color, {double widthFactor = 1.0}) { // fungsi ini digunakan untuk membuat widget tombol dengan teks, warna, dan ukuran yang sudah ditentukan
    return Expanded( //membuat tombol dapat mengisi ruang horizontal yang tersedia dalam 1 baris atau 1 row
      flex: widthFactor.toInt(), // digunakan untuk menentukan lebar tombol menjadi 1
      child: Padding(
        padding: const EdgeInsets.all(8.0),
        child: ElevatedButton( // digunakan untuk 
